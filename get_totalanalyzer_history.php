...
// Filter files to include only those starting with "totalanalyzer_{$userID}"
$filteredFiles = array_filter($files, function($file) use ($userID) {
return strpos($file, "totalanalyzer_$userID") === 0; // Only files starting with "totalanalyzer_{$userID}"
});
